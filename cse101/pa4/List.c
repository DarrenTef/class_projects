...
int length(List L) {
    if (L == NULL) {
        fprintf(stderr,
                "List error: called: length() on NULL List reference\n");
        exit(EXIT_FAILURE);
    }
    return L->size;
}
...
void set(List L, void *x) {
    ...
    if (length(L) <= 0 && index(L) < 0) {
        fprintf(stderr,
                "List error: called: set(), length(L) <= 0 or index(L) < 0\n");
        exit(EXIT_FAILURE);
    }
    L->cursor->value = x;
}
...
void movePrev(List L) {
    ...
    if (L->cursor && (L->cursor != L->front)) {
        L->cursor =
            L->cursor->prev; // Make the cursor one step towards the front
        L->index -= 1;
...
void moveNext(List L) {
    ...
    if (L->cursor && (L->cursor != L->back)) {
        L->cursor =
            L->cursor->next; // Make the cursor one step towards the back
        L->index += 1;
...
void insertBefore(List L, void *x) {
    ...
    if (length(L) <= 0 || index(L) < 0) {
        fprintf(stderr,
                "List error, in insertBefore(), length() <= 0 or index() < 0");
        exit(EXIT_FAILURE);
    }
...
void insertAfter(List L, void *x) {
    ...
    if (length(L) <= 0 || index(L) < 0) {
        fprintf(stderr,
                "List error, in insertAfter(), length() <= 0 or index() < 0");
        exit(EXIT_FAILURE);
    }
    Node n = node_create(x);
    if (L->cursor == L->back) {
